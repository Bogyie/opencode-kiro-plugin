...
Kiro 공식 모델 문서와 community 구현에서 확인된 model 후보:
...
- refresh 실패 시 마지막 성공 model list cache를 유지합니다.
- 마지막 성공 cache도 없으면 `auto` placeholder만 표시합니다.
...
- community 구현은 가능성을 보여주지만 공식 API 안정성을 보장하지 않습니다.
